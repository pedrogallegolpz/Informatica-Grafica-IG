...
void DescrTabla::activar_md()
{
   ...
   //PROPIO: si el VBO no está creado todavía en la GPU, crearlo y transferir los datos
   if ( nombre_vbo == 0 ){ 
   ...
   }else{ // El VBO está ya creado
      glBindBuffer(tipo_tabla, nombre_vbo);
   }


   if(tipo_tabla==GL_ARRAY_BUFFER){             // Si es tabla de atributos:
      fijarPuntero(0);                          //    especificar localización y formato. DUDA: el puntero es '0' ó es 'datos'
      glBindBuffer(tipo_tabla, 0);              //    desactiva el VBO
      glEnableClientState(atributo);            //    habilita uso de la tabla
   }
...
void ArrayVertices::visualizarGL_MI_BVE( const GLenum tipo_primitiva ) 
{
   ...
   // .........
   const int nv = (indices != nullptr) ? indices->num_tuplas_ind : num_vertices;     // es un if-else

   glBegin( tipo_primitiva);
   for( GLuint i = 0; i < nv; i++ ){
      // recuperar índice de vértice que corresponda
...
void ArrayVertices::visualizarGL_MI_DAE( const GLenum tipo_primitiva ) 
{
   ...
   glBindVertexArray(0);      // Activa el VAO por defecto
